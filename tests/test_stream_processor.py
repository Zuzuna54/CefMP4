...
@pytest.fixture
def stream_processor_instance(mock_file_path: Path) -> StreamProcessor:
...
@pytest.mark.asyncio
async def test_stream_processor_initialization(stream_processor_instance: StreamProcessor, mock_file_path: Path):
    ...
    assert not stream_processor_instance.is_processing
...
async def test_initialize_from_checkpoint_existing_stream(
    ...
    expected_parts_info = [{"PartNumber": i, "ETag": f"etag-{i}", "Size": 2048} for i in range(1, expected_next_part)]
...
async def test_process_file_write_new_data_single_chunk(
    ...
    mock_incr_bytes_sent.return_value = len(file_content) # Simulate Redis returning new total
    ...
    mock_add_part_info.assert_called_once_with(TEST_STREAM_ID, 1, "etag-part1", len(file_content))
    ...
    assert not stream_processor_instance.is_processing # Should be false after completion
...
async def test_process_file_write_multiple_chunks(
    ...
    # Part 1
    assert stream_processor_instance.uploaded_parts_info[0]["PartNumber"] == 1
    assert stream_processor_instance.uploaded_parts_info[0]["ETag"] == "etag-part1-multi"
    assert stream_processor_instance.uploaded_parts_info[0]["Size"] == settings.chunk_size_bytes
    # Part 2
    assert stream_processor_instance.uploaded_parts_info[1]["PartNumber"] == 2
    assert stream_processor_instance.uploaded_parts_info[1]["ETag"] == "etag-part2-multi"
    assert stream_processor_instance.uploaded_parts_info[1]["Size"] == expected_total_bytes - settings.chunk_size_bytes
    ...
    mock_add_part_info.assert_any_call(TEST_STREAM_ID, 1, "etag-part1-multi", settings.chunk_size_bytes)
    mock_add_part_info.assert_any_call(TEST_STREAM_ID, 2, "etag-part2-multi", expected_total_bytes - settings.chunk_size_bytes)
    
    assert mock_incr_bytes_sent.call_count == 2
    mock_incr_bytes_sent.assert_any_call(TEST_STREAM_ID, settings.chunk_size_bytes)
    mock_incr_bytes_sent.assert_any_call(TEST_STREAM_ID, expected_total_bytes - settings.chunk_size_bytes)
    ...
    assert mock_update_last_activity.call_count == 2 # Called after each successful part upload
    assert not stream_processor_instance.is_processing
...
async def test_process_file_write_s3_upload_failure(
    ...
    assert stream_processor_instance.current_file_offset == 0 # Should not advance offset on failure
    assert stream_processor_instance.next_part_number == 1 # Should not advance part number
    assert len(stream_processor_instance.uploaded_parts_info) == 0 # No part info should be added
    assert f"[{TEST_STREAM_ID}] Failed to upload part 1. Halting stream." in caplog.text
    assert not stream_processor_instance.is_processing # is_processing should be False
...
async def test_process_file_write_file_disappears_before_processing(
    ...
    assert f"[{TEST_STREAM_ID}] File {mock_file_path} no longer exists. Stopping processing." in caplog.text
    # TODO: Check for stream marked as failed in Redis (Phase 8)


@pytest.mark.asyncio
@patch('src.stream_processor.upload_s3_part', new_callable=AsyncMock) # Mock S3 to avoid actual calls
async def test_process_file_write_file_disappears_during_processing(
    ...
    def faulty_open(*args, **kwargs):
        if args[0] == stream_processor_instance.file_path and stream_processor_instance.next_part_number > 1:
            # On subsequent access, for the second chunk (or if trying to re-read after seek)
            if mock_file_path.exists(): # if it's for the second chunk, make it disappear
                 mock_file_path.unlink()
    ...
    assert stream_processor_instance.current_file_offset == settings.chunk_size_bytes # First chunk processed
    assert stream_processor_instance.next_part_number == 2 # Incremented after first successful upload
    assert len(stream_processor_instance.uploaded_parts_info) == 1
    assert stream_processor_instance.uploaded_parts_info[0]["ETag"] == "etag-part1-disappear"
    assert f"[{TEST_STREAM_ID}] Error processing file write for {mock_file_path}: Simulated read error after first chunk" in caplog.text
    assert not stream_processor_instance.is_processing
